...
const STORY_BASE_URL_BY_VARIANT: Record<string, string> = {
...
const BRAND_NAME_BY_VARIANT: Record<string, string> = {
  full: 'World Monitor',
  tech: 'Tech Monitor',
...
const STORY_BASE_URL = STORY_BASE_URL_BY_VARIANT[SITE_BRAND_VARIANT] ?? STORY_BASE_URL_BY_VARIANT.full;
const BRAND_NAME = BRAND_NAME_BY_VARIANT[SITE_BRAND_VARIANT] ?? BRAND_NAME_BY_VARIANT.full;
...
export function generateStoryDeepLink(
  ...
  return `${STORY_BASE_URL}/api/story?${params.toString()}`;
}
...
export const shareTexts = {
  ...
  linkedin: (data: StoryData) =>
    ...
    `\nData via ${BRAND_NAME} - Open source geopolitical intelligence`,
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  return {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} Intelligence Brief - ${BRAND_NAME}`)}`,
